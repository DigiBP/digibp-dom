...
#### Recommendations
To improve the process further, we would of course recommend adapting the process to accommodate tasks that can have multiple instantiations, but also the integration of an Alexa digital assistant, to enable higher process efficiency and make the overall process easier for those involved by increasing the amount of automation. For example, the chatbot could be used to interact with employees to schedule interview times, as this is typically a very time consuming subprocess with many interactions between HR and each applicant. The digital assistant would ask the candidate times they would be available, and the digital assistant could check if the interviewer(s) is/are available at this time (through an API integrated with their Outlook calendars for example).

Another area where efficiency could be gained is in sending out the rejection emails to candidates that get rejected as some point during the recruitment process. As you can see in the process flow description, several rounds of rejection emails must be sent and this is very time consuming for HR personnel. This step could be automated by have a generic email template that is personalized with each applicant's name e.g. "Dear Lauren", instead of "Dear Applicant".
